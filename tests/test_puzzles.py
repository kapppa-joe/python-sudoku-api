...
def test_puzzles_get_with_query(client):
    ...
    response = client.get('/puzzles?limit=15')
    assert response.status_code == 200
    ...
    list_before_offset = client.get('/puzzles').json["puzzles"]
    response = client.get('/puzzles?offset=5')
    ...
    response = client.get('/puzzles?sort_by=id&order=desc')
    response.status_code == 200
    puzzles = response.json["puzzles"]
    for i in range(len(puzzles) - 1):
        assert puzzles[i]["id"] > puzzles[i + 1]["id"]
    ...
    response = client.get('/puzzles?order=1234')
    assert response.status_code == 400
